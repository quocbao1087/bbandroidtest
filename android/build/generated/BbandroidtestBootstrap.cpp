...
static Handle<Value> Bbandroidtest_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Bbandroidtest.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::BbandroidtestBindings::lookupGeneratedInit(
...
static void Bbandroidtest_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Bbandroidtest_getBinding)->GetFunction());
}

static void Bbandroidtest_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::BbandroidtestBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry BbandroidtestBinding = {
	"bb.test",
	Bbandroidtest_init,
	Bbandroidtest_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_bb_test_BbandroidtestBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("bb.test", &BbandroidtestBinding);
	titanium::KrollBindings::addExternalLookup(&(::BbandroidtestBindings::lookupGeneratedInit));
}
